...
/*     [System.Serializable]
    ...
    public class BicycleController : BicycleMovement
    {
        ...
        private EventInstance bikeGravelSounds;
        private EventInstance bikeGearSounds;
        ...
        protected override void Start()
        {
            ...
            /* bikeGravelSounds = AudioManager.Instance.CreateEventInstance(FMODEvents.Instance.bikeGravelSFX);
            RuntimeManager.AttachInstanceToGameObject(bikeGravelSounds, Camera.main.transform);
            bikeGravelSounds.setParameterByName("PAR_SX_RidingOnGravel_Intensity", 1.0f);
            bikeGravelSounds.start(); */
/*             bikeGearSounds = AudioManager.Instance.CreateEventInstance(FMODEvents.Instance.bikeGearSFX);
            RuntimeManager.AttachInstanceToGameObject(bikeGearSounds, Camera.main.transform);
            bikeGearSounds.setParameterByName("PAR_SX_BicycleGears_Intensity", 1.0f);
            bikeGearSounds.start(); */
        }

        void FixedUpdate()
        {
            SteerBike(customSteerAxis, customLeanAxis, oscillationSteerEffect);
            ...
            // Gear Sound Logic, Doesn't really work right now

/*             if((currentSpeed / topSpeed) <= 0.3f) {
                // Set spoke sounds to true
                if((currentSpeed / topSpeed) < 0.05f) {
                    bikeGearSounds.setParameterByName("PAR_SX_BicycleGears_Intensity", 0.0f);
                } else {
                    bikeGearSounds.setParameterByName("PAR_SX_BicycleGears_Intensity", 1.0f);
                }
            } else {
            ...
            // adjust gravel sound parameter
            bikeGravelSounds.setParameterByName("PAR_SX_RidingOnGravel_Intensity", Mathf.Clamp01(currentSpeed / topSpeed));

            SetTopSpeed(false);

            MoveBike(rawCustomAccelerationAxis, customAccelerationAxis);
            ...
            //CycleOscillation
            if ((currentSpeed > 5 && isReversing == false) || isAirborne || isBunnyHopping)
        ...
        public void SetHalt(bool haltValue)
        {
            halt = haltValue;
        }
        ...
        void ApplyCustomInput()
        {
            if (wayPointSystem.recordingState == WayPointSystem.RecordingState.DoNothing || wayPointSystem.recordingState == WayPointSystem.RecordingState.Record)
            {
                ...
                CustomInput(movementInput.x, ref customSteerAxis, 5, 5, false);
                CustomInput(movementInput.y, ref customAccelerationAxis, 1, 1, false);
                CustomInput(movementInput.x, ref customLeanAxis, 1, 1, false);
                CustomInput(movementInput.y, ref rawCustomAccelerationAxis, 1, 1, true);

/*                 if(cameraController.isInThirdPersonMode()) {
                    if(inputManager.GetSprintInputPressed()) {
                        cameraController.GetThirdPersonCam().GetComponent<ThirdPersonCameraController>().ZoomIn();
                        cameraController.GetThirdPersonCam().GetComponent<ThirdPersonCameraController>().IncreaseShake();
                    } else if(inputManager.GetSprintInputReleased()) {
                        cameraController.GetThirdPersonCam().GetComponent<ThirdPersonCameraController>().ResetZoom();
                        cameraController.GetThirdPersonCam().GetComponent<ThirdPersonCameraController>().ResetShake();
                    }
                } */

                // Handling Sprinting
                // sprint = (inputManager.GetSprintInput() != 0);

                // Handling Hoping
/*                 if(inputManager.GetHopInput() != 0) {
                    bunnyHopInputState = 1;
                } else if(inputManager.PlayerHopReleasedThisFrame()) {
