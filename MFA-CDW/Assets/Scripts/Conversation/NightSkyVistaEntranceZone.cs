...
public class NightSkyVistaEntranceZone : MonoBehaviour
{
    ...
    [SerializeField] private int _numFovIncrements = 3;
    ...
    //Dexter
    [SerializeField] private VistaFadeable[] _appearingTexts;
    [SerializeField] private Image[] _traitTexts;
    [SerializeField] private string[] _traitList;
    [SerializeField] private Image[] _constellations;
    [SerializeField] private VistaFadeable[] _stars;

    //[SerializeField] private GameObject _explorePrompt;
    [SerializeField] private GameObject _reflectPrompt;
    private CinemachineVirtualCamera _cam;
    //[SerializeField] private VistaReticle[] _reticles; //TODO: mimic how it uses distance as reference for fading up/down "Trait" text and Stars
    ...
    [SerializeField] private GameObject _exitPrompt; //TODO: turn this on when players have completed certain number of traits, make it be needed text
    ...
    private List<string> _chosenTraits;
    ...
    private void Start()
    {
        ...
        _triggered = new bool[_appearingTexts.Length];
        _cam = GetComponent<CinemachineVirtualCamera>();
        _cam.m_Lens.FieldOfView = _startingFov;
        _targetFov = _startingFov;
        _inputManager = InputManager.Instance;
        foreach(Image constellation in _constellations)
        {
    ...
    {
        for (int i = 0; i < _traitTexts.Length; i++)
        {
            _traitTexts[i].enabled = false;
        }
    }
    ...
    public List<string> GetChosenTraits()
    {
        return _chosenTraits;
    }
    ...
    private void Update()
    {
        ...
        if (_firstTimeTriggered)
        {
            ...
            for(int i = 0; i < _triggered.Length; i++)
            {
                if (!_triggered[i])
                {
                    Vector3 screenPos = Camera.main.WorldToScreenPoint(_stars[i].transform.position); //TODO: refactor to work for any single one
                    Vector2 screenPos2D = new Vector2(screenPos.x, screenPos.y);
                    distFromCenter = Vector2.Distance(centerScreen, screenPos2D);
                    if (distFromCenter <= _selectRange)
                    {
                        float cursorDistance = Mathf.Clamp((_selectRange - distFromCenter) / (_selectRange - 10f), 0f, 1f);
                        _stars[i].SetCursorDistance(cursorDistance);
                        _appearingTexts[i].SetCursorDistance(cursorDistance);
                        ...
                        if (_breathObserver.IsOverThreshold && _breathObserver.hasJumped)
                        {
                            ...
                            _triggered[i] = true;
                            //StartCoroutine("StartFade", _thoughtsCompleted); //TODO: Here's where we confirm the selection
                            _constellations[i].enabled = true;
                            _chosenTraits.Add(_traitList[i]);
                            _thoughtsCompleted++;
                            canMove = false;
                            if (_thoughtsCompleted < _numFovIncrements + 1)
                            {
                                _targetFov += _fovIncrement;
                            }
                            if(_thoughtsCompleted == 1)
                            {
                                _exitPrompt.gameObject.SetActive(true);
                                _canLeaveVista = true;
                            }
                            /*if (_thoughtsCompleted == _disappearingText.Length && !textFinished) //Dexter: commenting this out to disable the auto-ending condition
                            {
